...
const SimplifiedDeltaverseService = require('../services/SimplifiedDeltaverseService');
...
const LoginScreen = ({ navigation }) => {
  ...
  const [useAPI, setUseAPI] = useState(true); // API toggle state
  const scrollViewRef = React.useRef(null);

  // Initialize data service based on toggle
  const [dataService] = useState(() => new SimplifiedDeltaverseService({ useAPI: true }));
  ...
  const handleAPIToggle = () => {
    ...
    // Switch data service mode
    if (newUseAPI) {
      dataService.switchToAPIMode();
      console.log('🌐 Switched to API mode');
    } else {
      dataService.switchToFileMode();
  ...
  const handleLogin = async () => {
    ...
    try {
      // Set the selected user in UserProfileService
      const success = UserProfileService.setCurrentUser(selectedUser);
      
      // Set in DataService (legacy)
      DataService.setCurrentUser(selectedUser);
      
      // Test the data service to ensure it's working
      const testUser = await dataService.getUserById(selectedUser);
      console.log(`Login: Using ${useAPI ? 'API' : 'File'} mode for user ${selectedUser}`);
      console.log(`User data source: ${testUser._source || 'unknown'}`);
      
      if (success) {
        // Navigate to main app with selected user and data service mode
      ...
      >
        ...
        <View style={styles.footer}>
          <Text style={[styles.footerText, { color: colors.textTertiary }]}>
            {useAPI ? 
              '🌐 Using live API data from deltaverse-api' : 
...
// Keep the same styles from original LoginScreen
